...
class VojageController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController
{
    ...
    protected $tabZone = array('1' => 'Marseille/Aix',
                                '2' => 'Hautes-Alpes',
                                '3' => 'Paris');
    //1 = Marseille - Aix
    //2 = Hautes-Alpes
                                    ...
                                    case '7': //arrivée Marseille
                                            ...
                                            //Le Queyras - Aiguilles ==> Aix-en-Pce TGV
                                                $this->addNewVoyage(
                                                    ...
                                                    14, //Aix-en-Pce TGV (destination location)
                                                            $this->request->getArgument('aixDate').' '.$this->request->getArgument('aixHour').':'.$this->request->getArgument('aixMinute')
                                                );
                                            ...
                                            //Vars ==> Aix-en-Pce TGV
                                                $this->addNewVoyage(
                                                    $this->request->getArgument('varsAdultPrice'),
                                                    $this->request->getArgument('varsChildPrice'),
                                                    14, //Vars (departure location)
                                                            $this->request->getArgument('varsDate').' '.$this->request->getArgument('varsHour').':'.$this->request->getArgument('varsMinute'),
                                                    14, //Aix-en-Pce TGV (destination location)
                                                            $this->request->getArgument('aixDate').' '.$this->request->getArgument('aixHour').':'.$this->request->getArgument('aixMinute')
                                                );
                                            //Vars ==> Marseille Aéroport
                                                $this->addNewVoyage(
                                                    $this->request->getArgument('varsAdultPrice'),
                                                    $this->request->getArgument('varsChildPrice'),
                                                    14, //Vars (departure location)
                    ...
                    case '2': //départ Marseille
                            //Aix-en-Pce TGV ==> Le Queyras - Aiguilles
                                $this->addNewVoyage(
                                    $this->request->getArgument('queyrasAdultPrice'),
                                    $this->request->getArgument('queyrasChildPrice'),
                                    3, //Aix-en-Pce TGV (departure location)
                                            $this->request->getArgument('aixDate').' '.$this->request->getArgument('aixHour').':'.$this->request->getArgument('aixMinute'),
                            ...
                            //Aix-en-Pce TGV ==> Vars
                                $this->addNewVoyage(
                                    ...
                                    3, //Aix-en-Pce TGV (departure location)
                                            $this->request->getArgument('aixDate').' '.$this->request->getArgument('aixHour').':'.$this->request->getArgument('aixMinute'),
                                    4, //Vars (destination location)
                                            $this->request->getArgument('varsDate').' '.$this->request->getArgument('varsHour').':'.$this->request->getArgument('varsMinute')
                                );
                            //Marseille Aéroport ==> Vars
                                $this->addNewVoyage(
                                    $this->request->getArgument('varsAdultPrice'),
                                    $this->request->getArgument('varsChildPrice'),
                                    1, //Marseille Aéroport (departure location)
...
//                $this->writeLog('vérification s\'il y a un retour'.chr(13), $this->path_log);
        $retour = '';
        if ($order->getToCity() != '') {
            $retour = '<br>Retour : Le '.$order->getToTime().' de '.$order->getToCity().' - '.$order->getToHour().' à '.$order->getToCity2().' - '.$order->getToHour2();
        }

        $detailMail = 'Bonjour,<br>Commande : '.$order->getUid().'<br>Ci-dessous le détail : <br><br>Nom : '.$order->getIdUsers()->getFirstName().' '.$order->getIdUsers()->getLastName().'<br>Email : '.$order->getIdUsers()->getEmail().'<br>Téléphone : '.$order->getIdUsers()->getTelephone().'<br>Adresse : '.$order->getIdUsers()->getAddress().'<br>Cp : '.$order->getIdUsers()->getZip().'<br>Ville : '.$order->getIdUsers()->getCity().'<br><br>Nombre d\'adulte : '.$order->getNbAdult().'<br>Nombre d\'enfants de 3 à 10 ans inclus : '.$order->getNbChildren().'<br>Enfants de moins de 3 ans : '.$order->getNbBaby().'<br>Tarif : '.$order->getPrice().' €<br><br>Aller : Le '.$order->getFromTime().' de '.$order->getFromCity().' - '.$order->getFromHour().' à '.$order->getFromCity2().' - '.$order->getFromHour2().$retour;

        //envoie du mail à l'admin
//                $this->writeLog('envoi du mail à l\'admin'.chr(13), $this->path_log);
        $contenuHTML = $detailMail;
        $this->sendMail('info@neigeexpress.com', 'Autocars Imbert', 'info@neigeexpress.com', '', '[ AUTOCARS IMBERT ] Nouvelle commande : '.$order->getUid(), $contenuHTML);

//                $this->writeLog('envoi du mail à nous'.chr(13), $this->path_log);
        $contenuHTML = $detailMail;
        $this->sendMail('info@neigeexpress.com', 'Autocars Imbert', 'gap@pimentrouge.fr', '', '[ AUTOCARS IMBERT ] Nouvelle commande : '.$order->getUid(), $contenuHTML);

        //envoie du mail au client
//                $this->writeLog('envoi du mail au client'.chr(13), $this->path_log);
        $contenuHTML = $detailMail.'<br><br><strong>Merci d\'imprimer ce mail et de le montrer au chauffeur avec une pièce d\'identité.<br>Attention, sans cette preuve d\'achat, le chauffeur pourra vous refuser l\'accès à l\'autocars.</strong>';
        $this->sendMail('info@neigeexpress.com', 'Autocars Imbert', $order->getIdUsers()->getEmail(), $order->getIdUsers()->getFirstName().' '.$order->getIdUsers()->getLastName(), '[ AUTOCARS IMBERT ] Commande : '.$order->getUid(), $contenuHTML);
    }
}
